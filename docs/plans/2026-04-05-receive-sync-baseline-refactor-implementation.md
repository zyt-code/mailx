...
- `AppShell.mailbox.test.ts` now also covers the no-account keyboard-refresh guard so `GetStarted` state does not emit `sync:trigger`
...
- focused frontend regression suite now passes at 36 files / 140 tests
