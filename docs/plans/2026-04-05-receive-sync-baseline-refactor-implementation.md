...
> **Status update (2026-04-06):** Core mailbox/sync extraction is in place, `AppShell`, `ReadingPane`, and `Sidebar` have been thinned further through dedicated helpers, `npm run check` is green, and the focused frontend regression suite currently passes at 36 files / 161 tests. Workflow integration coverage now includes settings entry, primary mobile mailbox flows, mobile ReadingPane delete/archive return-to-list behavior, mobile no-account `GetStarted` onboarding recovery, sidebar-driven mailbox transitions, the refresh-to-reload receive path, keyboard-refresh sync/reload coverage for aggregate and explicit mailbox scopes, the no-account keyboard refresh and compose guards in `GetStarted`, the no-account sync-orchestrator guard in the receive pipeline, stale-explicit-account sync intents being ignored after deletion, initial auto-sync sync/reload coverage on account activation, deferred auto-sync recovery when the active account changes during an in-flight sync, account lifecycle recovery coverage for `account:deleted`, current-or-unrelated `account:updated`, unrelated `account:deleted`, and `account:created`, active-account deletion fallback using the post-delete account set including the final-account non-inbox path, stale explicit mailbox selection fallback using the current configured accounts, desktop mailbox-context selection clearing on folder/account switches, and desktop continuous-reading selection after list-side delete, ReadingPane delete, ReadingPane archive, last-visible fallback mutations, and external mailbox updates removing the current selection, with remaining work focused on broader workflow-level coverage and any hardening that emerges from it.
...
- `AppShell.mailbox.test.ts` now also covers the no-account keyboard refresh and compose guards so `GetStarted` state does not emit `sync:trigger` or `compose:open`
- `AppShell.mobile-workflow.test.ts` now covers `list -> reading -> back`, the stale-selected-mail return-to-list flow on mobile, mobile sidebar folder/account transitions back into the mailbox list, ReadingPane delete/archive actions returning mobile users to the list, and the no-account mobile path rendering `GetStarted` instead of a mailbox shell
...
- `AppShell.sync-workflow.test.ts` now also covers the deferred-auto-sync path where an active-account change happens while the app is already syncing and the latest account must still receive its initial sync once syncing ends
- `autoSyncLifecycle.test.ts` now covers the pending-account handoff so active-account changes during an in-flight sync are not dropped
- `syncOrchestrator.test.ts` now covers the no-account guard so `sync:trigger` becomes a no-op instead of falling through to `syncAllAccounts`
- `syncOrchestrator.test.ts` now also covers stale explicit account payloads so deleted-account refresh intents are ignored instead of syncing an unknown mailbox
- `mailboxStore.test.ts` now covers active-account deletion before the `accountsStore` refresh so fallback selection and reload use the remaining accounts instead of the removed mailbox, including the last-account non-inbox case where reload must stay off the deleted mailbox
- `mailboxStore.test.ts` now also covers current and unrelated `account:updated` events so account metadata edits do not reload mailbox data
- `mailboxStore.test.ts` now also covers unrelated `account:deleted` events so removing another account does not reload the current explicit mailbox
- `mailboxScope.test.ts` and `mailboxStore.test.ts` now also cover stale explicit mailbox selections so unknown account ids are normalized back to a real configured account before reload
- `AppShell.account-workflow.test.ts` now covers desktop selection reset when an explicit mailbox account is deleted, mobile return-to-list when that account disappears mid-reading, direct non-inbox `account:deleted -> GetStarted` recovery when the last account disappears, current and unrelated `account:updated` events preserving the current reading session, unrelated `account:deleted` events preserving the current reading session, and `GetStarted -> mailbox list` recovery when an account is created
- `AppShell.selection-workflow.test.ts` now covers desktop continuous-reading selection for list-side delete, ReadingPane delete, ReadingPane archive, the fallback-to-previous path when the removed selection was the last visible mail, and the external-mailbox-update path where the selected mail disappears but another visible mail remains
- `appShellSelectedMail.ts` and `appShellSelectedMailLifecycle.ts` now preserve desktop reading continuity across external mailbox updates by choosing the nearest still-visible mail before clearing the selection
...
- focused frontend regression suite now passes at 36 files / 161 tests
