...
- `mailboxStore.test.ts` now covers active-account deletion before the `accountsStore` refresh so fallback selection and reload use the remaining accounts instead of the removed mailbox
...
- focused frontend regression suite now passes at 36 files / 153 tests
