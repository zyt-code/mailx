...
use crate::credentials_platform::credentials::platform::{PlatformCredentialError, PlatformResult};
#[cfg(target_os = "windows")]
use crate::credentials_platform::WindowsCredentialManager;
#[cfg(target_os = "windows")]
use crate::credentials_platform::EncryptedFileStore;
...
// ============================================================================
// P8-2 Test Suite: Windows Credential Management Integration Tests
// ============================================================================
...
// ============================================================================
// Encrypted File Store Tests (Fallback Backend)
// ============================================================================
...
mod encrypted_file_store_tests {
    ...
    // Test 1: Encrypted file store initialization
    #[test]
    fn test_encrypted_file_store_init() {
        let result = EncryptedFileStore::new();
        assert!(
            result.is_ok(),
            "EncryptedFileStore should initialize successfully"
        );
    }

    // Test 2: Store credential in encrypted file
    #[test]
    fn test_encrypted_file_store() {
        let store = EncryptedFileStore::new().unwrap();

        let account_id = test_account_id("encrypted_store");
        let password = test_password();

        let store_result = store.store_credential(&account_id, &password);
        assert!(
            store_result.is_ok(),
            "Should store credential in encrypted file"
        );

        let retrieve_result = store.get_credential(&account_id);
        assert!(
            retrieve_result.is_ok(),
            "Should retrieve from encrypted file"
        );
        assert_eq!(
            retrieve_result.unwrap(),
            password,
            "Encrypted file should preserve password"
        );

        // Cleanup
        let delete_result = store.delete_credential(&account_id);
        assert!(
            delete_result.is_ok(),
            "Should delete from encrypted file"
        );
    }
    ...
    fn test_encryption_decryption_roundtrip() {
        ...
        let account_id = test_account_id("roundtrip");
        let original_password = "Test@Password123!#$%";

        // Store
        store.store_credential(&account_id, original_password)
            .unwrap();

        // Retrieve
        let retrieved = store.get_credential(&account_id).unwrap();

        assert_eq!(
            retrieved, original_password,
            "Encryption/decryption should be lossless"
        );

        // Cleanup
        store.delete_credential(&account_id).unwrap();
    }

    // Test 4: Multiple credentials isolation
    #[test]
    fn test_multiple_credentials_isolation() {
        let store = EncryptedFileStore::new().unwrap();

        let account1 = test_account_id("iso1");
        let account2 = test_account_id("iso2");
        let password1 = test_password();
        let password2 = test_password();

        // Store both
        store.store_credential(&account1, &password1).unwrap();
        store.store_credential(&account2, &password2).unwrap();

        // Retrieve and verify isolation
        let retrieved1 = store.get_credential(&account1).unwrap();
        let retrieved2 = store.get_credential(&account2).unwrap();

        assert_eq!(retrieved1, password1, "Account 1 password should match");
        assert_eq!(retrieved2, password2, "Account 2 password should match");
        assert_ne!(retrieved1, retrieved2, "Passwords should be different");

        // Cleanup
        store.delete_credential(&account1).unwrap();
        store.delete_credential(&account2).unwrap();
    }
}
...
#[cfg(target_os = "windows")]
mod platform_abstraction_tests {
    use super::*;

    // Test 1: Platform error types
    #[test]
    fn test_platform_error_types() {
        let error = PlatformCredentialError::NotFound("test_account".to_string());
        assert!(
            error.to_string().contains("not found"),
            "NotFound error should contain 'not found'"
        );

        let error = PlatformCredentialError::StorageFailed("test failure".to_string());
        assert!(
            error.to_string().contains("Storage operation failed"),
            "StorageFailed error should contain 'Storage operation failed'"
        );
    }

    // Test 2: Platform result type
    #[test]
    fn test_platform_result_type() {
        fn returns_success() -> PlatformResult<String> {
            Ok("success".to_string())
        }

        fn returns_error() -> PlatformResult<String> {
            Err(PlatformCredentialError::NotFound("test".to_string()))
        }
...
// ============================================================================
// Non-Windows Tests (ensure cross-platform compatibility)
// ============================================================================

#[cfg(not(target_os = "windows"))]
mod non_windows_tests {
    #[test]
    fn test_windows_specific_tests_skipped() {
        // On non-Windows platforms, Windows-specific tests are not compiled
        // This test ensures the test suite still runs on other platforms
        assert!(true, "Non-Windows platform: Windows tests skipped");
    }
}
