...
mod tests {
    ...
    fn create_test_database() -> (Database, TempDir) {
        ...
        let db = Database {
            ...
            attachments_root: temp_dir.path().join("attachments"),
        };
        (db, temp_dir)
    }
    ...
    fn test_cleanup_old_notifications_30_days() {
        ...
        .unwrap();
        assert_eq!(
            db.get_notifications(Some(account_id), 100).unwrap().len(),
            3
        );
    ...
    fn test_get_notifications_by_account() {
        ...
        for i in 1..=5 {
            db.insert_notification(
                ...
                Some(i as i64),
                "account1_type",
                &format!("Account 1 - Notification {}", i),
                "Body for account 1",
                1,
                None,
            )
        ...
        for i in 1..=3 {
            db.insert_notification(
                ...
                Some(i as i64),
                "account2_type",
                &format!("Account 2 - Notification {}", i),
                "Body for account 2",
                1,
                None,
            )
        ...
        assert_eq!(
            db.get_notifications(Some(account1_id), 100).unwrap().len(),
            5
        );
        assert_eq!(
            db.get_notifications(Some(account2_id), 100).unwrap().len(),
            3
        );
