...
import {
  ...
  syncNotificationPreferencesToBackend
	...
	// 监听后端通知事件
	let unlistenNotification: (() => void) | undefined = undefined;
...
async function sendTestAlert() {
	console.log('[Test] 📤 Sending test notification with Tauri plugin...');
	
	try {
		permissionMessage = '正在发送测试通知...';
		
		// 使用新的命令（使用Tauri官方插件）
		await invoke('send_notification_with_history', {
